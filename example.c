...
{ //lдобавить условие если в очереди есть люди то грузить их
	a=-1;//помогает понять какой поток допустить к мьютексу
	b[6]=i;//рандом число
	b[8] = 4; 
	if (b[4]+b[5]==2*N && b[7] == 0)//если паром полон и на причале
	{
		b[4]=0;//обнуляем переменную колва апл
		b[5]=0;//и майкров на пароме
		b[7]=1;//и отправляем его
	}
	else if (b[4]+b[5]<2*N) b[7]=0;//если не хватает пока, то паром стоит
		if (i <= 0)//запоолняем эпл
		{
				b[0]++;//всего сколько их пришло
				b[2]++;//и сколько их в очереди
			if ((b[4]<=b[5] && b[5]<=N && b[4]<N) || (b[5]==0 && b[4]<2*N)) // два случая : либо едут и те и те, заполняем равномерно, либо едут только эпл
			{
			b[4] ++;//загоняем на паром
			b[2]--;//пошли из оереди
			}
		}
		else if (i > 0)// если же пришел майк, то проделывает с ним тоже самое
			{
				b[1]++;
				b[3]++;
...
int get_buffer(int i) //вытаскиваем нужную цифру из буффера
{ 
	a=1;
	int v;
	v = b[i]; 
...
//rand1 = rand()%361 + 1;
	...
	if (a == -1)//тут если а=-1 то мы ждем, иначе разблокируем мьютекс для определенного потока (или наоборот)
...
//for(int i=0; time(NULL)<20000;i++)
	...
	add_buffer(rand1); //закидывает кого-нить в очередь
...
void *consumer()
 { int all_Apple,all_Microsoft,count_A,count_M,Q1,Q2;int rand1;int i=0;int count;
	while (1)//типо пока нет ошибки
	{
		...
		//здесь присваеваем все что можно, хотя лучше напрямую обращаться, главное чтобы проблем не было
...
//printf("Пришел сотрудник Apple №%i ",b[0]);
	...
	{ 
    	...
    	TIME = 0;
//здесь идет заполнение только очереди
    	if (rand1 <= 0) 
	...
	else if (count == 0)//здесь идет заполнение парома и очереди
	{ 
    	if (rand1 <= 0) 
        printf("Пришел сотрудник Apple №%i ", rand1, all_Apple);
    	else if (rand1 > 0) 
